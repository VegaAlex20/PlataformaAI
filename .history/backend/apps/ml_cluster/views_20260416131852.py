...
def obtener_estrategico(request):
    try:
        ...
        # ⚠️ usamos ID_FECHA_REGISTRO_ENVIO (INT tipo 20240101)
        query = """
        SELECT
            f.ID_CLIENTE,
            ...
            SUM(f.MONTO_FACTURADO_BOB) AS GASTO_TOTAL
        FROM PROYECTO_BI_TRANSPORTE_V2.DWH_DEV.FACT_ENVIO f
        GROUP BY f.ID_CLIENTE, f.ID_FECHA_REGISTRO_ENVIO
        """
        ...
        df["FECHA"] = pd.to_datetime(
            df["ID_FECHA_REGISTRO_ENVIO"], format="%Y%m%d")
        df["MES"] = df["FECHA"].dt.to_period("M").astype(str)

        # ===== AGRUPACIÓN POR CLIENTE =====
        df_cliente = df.groupby("ID_CLIENTE").agg({
            "TOTAL_ENVIOS": "sum",
            "GASTO_TOTAL": "sum"
        }).reset_index()

        # ===== ML =====
        X = df_cliente.drop(columns=["ID_CLIENTE"])
        ...
        # ===== SEGMENTOS =====
        resumen = df_cliente.groupby("cluster").mean()
        ...
        resumen["SEGMENTO"] = resumen.apply(nombrar, axis=1)

        mapa = resumen["SEGMENTO"].to_dict()
        df_cliente["SEGMENTO"] = df_cliente["cluster"].map(mapa)
        ...
        ingresos = (
            df_cliente.groupby("SEGMENTO")["GASTO_TOTAL"]
            .sum()
            .to_dict()
        )
        ...
        # ===== EVOLUCIÓN MENSUAL =====
        df_merge = df.merge(
            df_cliente[["ID_CLIENTE", "SEGMENTO"]], on="ID_CLIENTE")

        evolucion = (
            df_merge.groupby(["MES", "SEGMENTO"])["GASTO_TOTAL"]
            .sum()
            .reset_index()
        )
