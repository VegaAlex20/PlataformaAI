...
def obtener_estrategico(request):
    try:
        ...
        HAVING SUM(f.MONTO_FACTURADO_BOB) > 0
        LIMIT 5000
        """
        ...
        df["SEGMENTO"] = pd.qcut(
            df["GASTO_TOTAL"],
            q=3,
            labels=["Regulares", "Frecuentes", "VIP"]
        )
        ...
        # ===== INGRESOS =====
        ingresos = df.groupby("SEGMENTO")["GASTO_TOTAL"].sum().to_dict()

        # ===== FECHAS CORRECTAS =====
        df["FECHA"] = pd.to_datetime(
            df["ID_FECHA_REGISTRO_ENVIO"].astype(str),
            format="%Y%m%d"
        )
        ...
        # 🔴 DEBUG
        print(df[["MES", "SEGMENTO", "GASTO_TOTAL"]].head())
