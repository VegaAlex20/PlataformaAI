...
def predecir_riesgo(request):
    if request.method == "POST":
        ...
        # ===== RECONSTRUCCIÓN DE VARIABLES DEL MODELO =====
        # (esto es clave para no romper tu modelo)
        costo_operativo = distancia * 1.5 + peso * 0.05
        costo_combustible = distancia * 0.8

        # ===== DATAFRAME FINAL (lo que espera el modelo) =====
        df = pd.DataFrame([{
        ...
        # ===== AJUSTE INTELIGENTE (esto es lo pro 💀) =====
...
def analisis_tactico(request):
    ...
    conductores.columns = ["ID_CONDUCTOR",
                           "RIESGO_PROMEDIO", "CASOS_RIESGO", "VIAJES"]
