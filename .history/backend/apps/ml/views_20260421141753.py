...
def analisis_tactico(request):
    ...
    top_riesgo = conductores.sort_values(
        "RIESGO_PROMEDIO", ascending=False
    ).head(10)

    # top buenos
    top_buenos = conductores.sort_values(
        "RIESGO_PROMEDIO", ascending=True
    ).head(10)
    ...
    return JsonResponse({
        ...
        # opcional para debug
        "viajes": df.head(50).to_dict(orient="records")
