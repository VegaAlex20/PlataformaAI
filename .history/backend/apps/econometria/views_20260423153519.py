...
def entrenar_modelo():
    ...
    # 🔴 IMPORTANTE: copiar antes de transformar
    df_modelo = df.copy()

    df_modelo["LOG_MONTO"] = np.log(df_modelo["MONTO_FACTURADO_BOB"] + 1)
    df_modelo["LOG_PESO"] = np.log(df_modelo["PESO_ENVIO_KG"] + 1)
    df_modelo["LOG_DISTANCIA"] = np.log(df_modelo["DISTANCIA_KM"] + 1)

    df_modelo = pd.get_dummies(
        df_modelo, columns=["SEGMENTO"], drop_first=True)
    df_modelo = pd.get_dummies(
        df_modelo, columns=["CIUDAD_DESTINO"], drop_first=True)
    ...
    variables += [c for c in df_modelo.columns if "SEGMENTO_" in c]
    variables += [c for c in df_modelo.columns if "CIUDAD_DESTINO_" in c]

    df_final = df_modelo[["LOG_MONTO"] + variables].dropna()
    ...
    return modelo, X.columns, df  # 🔴 DEVOLVEMOS df ORIGINAL
...
def resumen(request):
    try:
        seg = df_global.groupby("SEGMENTO")[
            "MONTO_FACTURADO_BOB"].mean().sort_values(ascending=False)
        ciudad = df_global.groupby("CIUDAD_DESTINO")[
            "MONTO_FACTURADO_BOB"].mean().sort_values(ascending=False)
        ...
        return JsonResponse({
            ...
            "top_segmentos": seg.head(5).to_dict(),
            "top_ciudades": ciudad.head(5).to_dict()
        })
