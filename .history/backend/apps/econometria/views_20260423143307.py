...
def entrenar_modelo():
    ...
    variables += [col for col in df.columns if "SEGMENTO_" in col]
    variables += [col for col in df.columns if "CIUDAD_DESTINO_" in col]
...
def predecir(request):
    ...
    return JsonResponse({
        "ingreso_estimado": float(pred_real)
    })
...
def resumen(request):
    return JsonResponse({
        ...
        "por_segmento": df_global.groupby("SEGMENTO")["MONTO_FACTURADO_BOB"].mean().to_dict(),
        "por_ciudad": df_global.groupby("CIUDAD_DESTINO")["MONTO_FACTURADO_BOB"].mean().to_dict()
    })
